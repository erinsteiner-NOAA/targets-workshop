...
Similarly, let's put the `library()` calls in their own script in `R/` called `packages.R` (this isn't the only way to do it though; see the ["Managing Packages" episode](https://carpentries-incubator.github.io/targets-workshop/packages.html) for alternative approaches).
